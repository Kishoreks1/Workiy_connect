...
public class LeaveManageController {
	...
	public ModelAndView submitApplyLeave(ModelAndView mav, @Valid LeaveDetails leaveDetails, BindingResult bindingResult) {
		...
		if (bindingResult.hasErrors()) {
			...
			//			}
			sendApplyLeaveEmail(userInfo.getEmail());
		}
		return mav;
	}
		...
		if (bindingResult.hasErrors()) {
			...
			if (fromdate.isAfter(todate)) {
				...
				if(newBalance<0){
					...
					mav.setView(new RedirectView("/user/lms"));
					sendEditLeaveEmail(userInfo.getEmail());
				}
			}
		}
		return mav;
	...
	public ModelAndView cancelLeave(@PathVariable("id") int id, ModelAndView mav) {
		...
		if (leaveDetails != null && leaveDetails.isActive()) {
			...
			sendCancledLeaveEmail(userInfo.getEmail());
		} else {
	...
	public ModelAndView withdrawLeave(@PathVariable("id") int id, ModelAndView mav) {
		...
		if (leaveDetails != null && leaveDetails.isAcceptRejectFlag()) {
			...
			sendWithdrawLeaveEmail(userInfo.getEmail());
		}
		mav.setView(new RedirectView("/user/lms"));
		return mav;
	}
	...
	public ModelAndView acceptOrRejectLeaves(ModelAndView mav, @PathVariable("action") String action,
		...
		if (action.equals("accept")) {
			...
			sendAcceptLeaveEmail(userInfo.getEmail());
		}
			...
			sendWithdrawAcceptLeaveEmail(userInfo.getEmail());
		}
		else if(action.equals("reject-withdraw")) {
			leaveDetails.setwithdraw(false);
			sendWithdrawRejectLeaveEmail(userInfo.getEmail());
		}
			...
			leaveDetails.setAcceptRejectFlag(false);
			leaveDetails.setActive(false);
			sendRejectLeaveEmail(userInfo.getEmail());
		}
	...
	private void sendApplyLeaveEmail(String to) {

		String subject = "Employee Registered";
		String templateName = EmailService.EMPLOYEE_APPLY_LEAVE;

		// Call the general sendEmail method from EmailService
		emailService.sendEmail(to, subject, templateName);
	}

	private void sendEditLeaveEmail(String to) {

		String subject = "Employee Registered";
		String templateName = EmailService.EMPLOYEE_EDIT_LEAVE;

		// Call the general sendEmail method from EmailService
		emailService.sendEmail(to, subject, templateName);
	}
	private void sendCancledLeaveEmail(String to) {

		String subject = "Employee Registered";
		String templateName = EmailService.EMPLOYEE_CANCLED_LEAVE;

		// Call the general sendEmail method from EmailService
		emailService.sendEmail(to, subject, templateName);
	}

	private void sendRejectLeaveEmail(String to) {

		String subject = "Employee Registered";
		String templateName = EmailService.EMPLOYEE_REJECT_LEAVE;

		// Call the general sendEmail method from EmailService
		emailService.sendEmail(to, subject, templateName);
	}

	private void sendAcceptLeaveEmail(String to) {

		String subject = "Employee Registered";
		String templateName = EmailService.EMPLOYEE_ACCEPT_LEAVE;

		// Call the general sendEmail method from EmailService
		emailService.sendEmail(to, subject, templateName);
	}
	private void sendWithdrawLeaveEmail(String to) {

		String subject = "Employee Registered";
		String templateName = EmailService.EMPLOYEE_WITHDRAW_LEAVE;

		// Call the general sendEmail method from EmailService
		emailService.sendEmail(to, subject, templateName);
	}
	private void sendWithdrawAcceptLeaveEmail(String to) {

		String subject = "Employee Registered";
		String templateName = EmailService.EMPLOYEE_WITHDRAW_ACCEPT_LEAVE;

		// Call the general sendEmail method from EmailService
		emailService.sendEmail(to, subject, templateName);
	}
	private void sendWithdrawRejectLeaveEmail(String to) {

		String subject = "Employee Registered";
		String templateName = EmailService.EMPLOYEE_WITHDRAW_REJECT_LEAVE;
